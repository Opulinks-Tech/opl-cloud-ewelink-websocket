...
extern IoT_Ring_buffer_t g_stIotRbData;
extern osTimerId g_cloud_disconnect_timeout_timer;
...
{
    printf("Rsp Padload = %s\r\n", (char *)u8pProperty_Payload);
...
//    }
    ...
    if(ret>0)
    {
        printf("[ATS]WIFI Send Reply success\r\n");
    }
    else
    {
        printf("[ATS]WIFI Send Reply fail\r\n");
...
void Iot_Post_Data_Construct(IoT_Properity_t *ptProperity , uint8_t *pu8Data, uint32_t *pu32Len)
{
    ...
    int8_t rssi = wpa_driver_netlink_get_rssi() + BLEWIFI_COM_RF_RSSI_ADJUST;
...
//    if( pstSensorData->ubaType==2 || pstSensorData->ubaType==3 )
    {
        ...
        if (already_wrt_flag)
            u32Offset += sprintf( ps8Buf +u32Offset, ",");
        u32Offset += sprintf( ps8Buf +u32Offset, "\"motion\":%u", pstSensorData->ubaPIRStatus);
        already_wrt_flag = 1;

#ifdef __IDR_EN__  //from build config
        if (already_wrt_flag)
            u32Offset += sprintf( ps8Buf +u32Offset, ",");
        u32Offset += sprintf( ps8Buf +u32Offset, "\"light\":%u", pstSensorData->ubaIDRstatus);
        already_wrt_flag = 1;
#endif
...
//            printf("defense: %d\r\n", u8Defense);
            ...
            App_Sensor_Set_Defense(u8Defense);
        }


        if (!lite_cjson_object_item(&tParams, "enable", 6, &tEnable)) {

            s8Enable = tEnable.value_int;
//            printf("enable: %d\r\n", s8Enable);
        }

        if (!lite_cjson_object_item(&tParams, "switch", 6, &tSwitch)) {
...
//            printf("Switchon: %d\r\n", s8Switchon);
        ...
        if (!lite_cjson_object_item(&tParams, "time", 4, &tTime)) {
