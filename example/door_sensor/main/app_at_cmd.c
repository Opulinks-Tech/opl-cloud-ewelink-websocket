...
int app_at_cmd_sys_get_voltage(char *buf, int len, int mode)
{
    ...
    switch (mode)
    {
        case AT_CMD_MODE_READ:
        {
            BleWifi_Ble_AdvertisingIntervalChange(BLEWIFI_BLE_ADVERTISEMENT_INTERVAL_CAL_MIN);
...
int app_at_cmd_sys_voltage_offset(char *buf, int len, int mode)
{
    ...
    switch (mode)
    {
        ...
        case AT_CMD_MODE_SET:
        {
            ...
            BleWifi_Ble_AdvertisingIntervalChange(BLEWIFI_BLE_ADVERTISEMENT_INTERVAL_PS_MIN);
...
int app_at_cmd_sys_read_switch(char *buf, int len, int mode)
{
    ...
    switch (mode)
    {
        case AT_CMD_MODE_READ:
        {
            if ( true == BleWifi_COM_EventStatusGet(g_tAppCtrlEventGroup, APP_CTRL_EVENT_BIT_DOOR))
                iRet = 1;
            else
                goto done;
...
int app_at_cmd_sys_factory(char *buf, int len, int mode)
{
    ...
    {
        App_Ctrl_MsgSend(APP_CTRL_MSG_IS_TEST_MODE_AT_FACTORY, NULL, 0);
    }
...
static void Wifi_Conn_Start(char *ssid, char *password)
{
    ...
    BleWifi_COM_EventStatusSet(g_tAppCtrlEventGroup , APP_CTRL_EVENT_BIT_AT_WIFI_MODE , true);
    ...
    stScanConfig.show_hidden = 1; // Enable to scan AP whose SSID is hidden
    stScanConfig.scan_type = WIFI_SCAN_TYPE_MIX;  // mixed mode
...
int app_at_cmd_sys_ck_cfg(char *buf, int len, int mode)
{
    int iRet = 0;
    int argc = 0;
    char *argv[AT_MAX_CMD_ARGS] = {0};
    ...
    switch (mode)
    {
        case AT_CMD_MODE_READ:
        {
            ...
            msg_print_uart1("Apikey:  \t%s\r\n", tDev.ubaApiKey);
            msg_print_uart1("DeviceID:\t%s\r\n", tDev.ubaDeviceId);
            msg_print_uart1("ChipID:  \t%s\r\n", tDev.ubaChipId);
            msg_print_uart1("ModelID: \t%s\r\n", tDev.ubaModelId);
            break;
        }
        ...
        case AT_CMD_MODE_SET:
        {
            ...
            {
                msg_print_uart1("Invalid APIKey Length\r\n");
                goto done;
            }
...
at_command_t g_taAppAtCmd[] =
{
    ...
    { "at+ckcfg",       app_at_cmd_sys_ck_cfg,             "CoolKit Configuration" },
