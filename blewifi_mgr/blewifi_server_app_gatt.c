...
static int32_t BleWifi_Ble_ServerAppGattMsgHandler_NotifyCfm(TASK task, MESSAGEID id, MESSAGE message);
static int32_t BleWifi_Ble_ServerAppGattMsgHandler_InitCfm(TASK task, MESSAGEID id, MESSAGE message);
static int32_t BleWifi_Ble_ServerAppGattMsgHandler_AccessReadInd(TASK task, MESSAGEID id, MESSAGE message);
static int32_t BleWifi_Ble_ServerAppGattMsgHandler_AccessWriteInd(TASK task, MESSAGEID id, MESSAGE message);
static int32_t BleWifi_Ble_ServerAppGattMsgHandler_ExchangeMtuInd(TASK task, MESSAGEID id, MESSAGE message);
static int32_t BleWifi_Ble_ServerAppGattMsgHandler_ExchangeMtuCfm(TASK task, MESSAGEID id, MESSAGE message);
static int32_t BleWifi_Ble_ServerAppGattMsgHandler_ConfirmationCfm(TASK task, MESSAGEID id, MESSAGE message);
static int32_t BleWifi_Ble_ServerAppGattMsgHandler_OperationTimeout(TASK task, MESSAGEID id, MESSAGE message);
...
static void BleWifi_Ble_AppHandleBwpServiceWrite(LE_GATT_MSG_ACCESS_WRITE_IND_T *ind)
{
	...
	switch (attrid)
    {
		case BWP_IDX_DATA_IN_VAL:
		{
            ...
            App_Ctrl_MsgSend(APP_CTRL_MSG_BLE_DATA_IND, ind->pVal, ind->len);
        }
		break;
...
static int32_t BleWifi_Ble_ServerAppGattMsgHandler_NotifyCfm(TASK task, MESSAGEID id, MESSAGE message)
{
    ...
    BleWifi_Ble_SendAppMsgToBle(BW_APP_MSG_SEND_TO_PEER_CFM, 0, NULL);
    return 0;
}
