...
/** @brief This structure use for mac_addr_get_config_source
*/
typedef struct {
    mac_iface_t iface;
    mac_source_type_t *type;
}__attribute__((packed)) blewifi_wifi_get_config_source_t;

/** @brief This structure use for mac_addr_set_config_source
*/
typedef struct {
    mac_iface_t iface;
    mac_source_type_t type;
}__attribute__((packed)) blewifi_wifi_set_config_source_t;

/** @brief This structure use for wifi_scan_get_ap_records
*/
typedef struct {
    uint16_t *pu16apCount;
    wifi_scan_info_t *pstScanInfo;
}__attribute__((packed)) blewifi_wifi_get_ap_record_t;

/** @brief This structure use for wifi_auto_connect_get_ap_info
*/
typedef struct {
    uint8_t u8Index;
    wifi_auto_connect_info_t *pstAutoConnInfo;
}__attribute__((packed)) blewifi_wifi_get_auto_conn_ap_info_t;

/** @brief This structure use for _BleWifi_Wifi_SetDTIM
*/
typedef struct {
    uint32_t u32DtimValue;
    uint32_t u32DtimEventBit;
}__attribute__((packed)) blewifi_wifi_set_dtim_t;

typedef enum {
    BLEWIFI_WIFI_QUERY_AP_INFO               = 0x0,
    BLEWIFI_WIFI_QUERY_GET_MAC_ADDR          = 0x1,
    BLEWIFI_WIFI_QUERY_GET_MANUFACTURER_NAME = 0x2,
    BLEWIFI_WIFI_GET_CONFIG_SOURCE           = 0x3,

    BLEWIFI_WIFI_QUERY_GET_RSSI              = 0x10,
    BLEWIFI_WIFI_GET_AP_NUM                  = 0x11,
    BLEWIFI_WIFI_GET_AP_RECORD               = 0x12,
...
typedef enum {
    BLEWIFI_WIFI_SET_MAC_ADDR          = 0x1,
    BLEWIFI_WIFI_SET_MANUFACTURER_NAME = 0x2,
    BLEWIFI_WIFI_SET_CONFIG_SOURCE     = 0x3,
    BLEWIFI_WIFI_SET_DTIM              = 0x20,
